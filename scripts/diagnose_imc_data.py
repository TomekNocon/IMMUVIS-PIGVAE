...
def analyze_normalization(samples, norm_type="channel_wise"):
    ...
    # With normalization
    transform_with_norm = IMCBaseDictTransform(normalize=True, norm_type=norm_type)
    ...
    for stat in stats_before[:5]:  # Show first 5
        print(
            f"{stat['sample']:<8} {stat['key']:<15} {stat['mean']:>11.4f} {stat['std']:>11.4f} "
    ...
    print(
        f"✅ STATUS: {'PASS' if abs(np.mean(means_after)) < 0.1 and 0.8 < np.mean(stds_after) < 1.2 else 'FAIL'}"
    )
...
def visualize_distributions(samples):
    ...
    for key in result_no_norm:
        if isinstance(result_no_norm[key], torch.Tensor):
            ...
            axes[0, 0].axvline(
                data_before.mean() + data_before.std(),
                color="green",
                linestyle="--",
                label=f"Std: {data_before.std():.2f}",
            )
            axes[0, 0].axvline(
                data_before.mean() - data_before.std(), color="green", linestyle="--"
            )
            axes[0, 0].legend()
            ...
            axes[0, 1].axvline(
                data_after.mean() + data_after.std(),
                color="red",
                linestyle="--",
            ...
            axes[1, 1].set_title(
                "Q-Q Plot After (should be more linear)", fontsize=12, fontweight="bold"
            )
