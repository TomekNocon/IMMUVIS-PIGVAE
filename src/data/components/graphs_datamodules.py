...
class PatchAugmentations(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "grid",
            self.make_grid(num_nodes_per_dim),
            persistent=False,
        )
...
class DenseGraphBatch:
    ...
    def take_sample(self, n) -> DenseGraphBatch:
        ...
        mask_opt = self.mask
        properties_opt = self.properties
        argsort_augmented_features = self.argsort_augmented_features
        perms_opt = self.perms

        mask_slice = mask_opt[:n, :] if mask_opt is not None else None
        perms_slice = perms_opt[:n, :] if perms_opt is not None else None
        properties_slice = properties_opt[:n] if properties_opt is not None else None
