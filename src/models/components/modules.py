...
class GraphEncoder(torch.nn.Module):
    ...
    def init_message_matrix(
        ...
        x = self.dropout(
            F.gelu(self.fc_in(node_features))
        )  # altenatives are silu - but works worse with negative values or no activation
        return x, mask
...
class SimplePermuter(torch.nn.Module):
    def __init__(self, hparams: DictConfig):
        ...
        self.register_buffer(
            "perm_matrices", self._precompute_permutation_matrices(hparams.grid_size)
        )
    ...
    def forward(
        ...
        if self.break_symmetry_scale > 0.1:
            ...
            warnings.warn(
                f"break_symmetry_scale={self.break_symmetry_scale} capped to 0.1", stacklevel=2
            )
        ...
        node_features = self.graph_transformer(
            node_features, mask=mask, is_encoder=False
        )  # does not matter I pass mask so 1 will be added
...
class BottleNeckDecoder(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = F.gelu(self.w(x))  # adds non-linearity before decoder
        return x
